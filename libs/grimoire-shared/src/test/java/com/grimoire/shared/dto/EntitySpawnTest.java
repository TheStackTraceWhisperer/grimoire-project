...
class EntitySpawnTest {
    ...
    @Test
    void testEntitySpawnDefensivelyCopiesComponents() {
        List<ComponentDTO> source = new ArrayList<>();
        source.add(new PositionDTO(100, 200));

        EntitySpawn spawn = new EntitySpawn("entity-123", source);
        source.clear();

        assertEquals(1, spawn.allComponents().size());
        assertThrows(UnsupportedOperationException.class,
                () -> spawn.allComponents().add(new PositionDTO(300, 400)));
    }
}
