...
All domain components are immutable records implementing the `Component` marker interface. In this ECS architecture, component records serve as value objects, and entity identity is a runtime concern (managed in the application/infra layer).
...
- `ComponentPropertyTest` — 14 `@Property` methods validating record construction, value equality, and `Component` interface membership
...
- `NavigationPropertyTest` (9 `@Property` methods) — grid round-trips, A* always finds path on empty grid, paths never traverse blocked cells, same-start-target returns empty, smoothed path ≤ original, line-of-sight symmetry
- `SpatialGridPropertyTest` (6 `@Property` methods) — insert-then-query, remove-then-absent, entity count, zone isolation, clear, position update
