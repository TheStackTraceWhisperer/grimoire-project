...
Full MMO-lite game; multi-module Maven, Java 21, Micronaut 4.8.3, Netty, Keycloak, PostgreSQL/JPA, JavaFX + LWJGL
...
- Two client implementations: JavaFX (`grimoire-client`) with OAuth2 auth flow, LWJGL (`grimoire-client-v2`) with OpenGL
...
- `GameServer` as `AutoCloseable` with `CompletableFuture`-based async `start()`/`stop()` and `ReentrantLock` state
  guards
- Full quality pipeline: SpotBugs + FindSecBugs, PMD + CPD, Checkstyle, merged JaCoCo (unit + IT, 80% line / 60%
  branch), ArchUnit, jqwik property tests, Formatter
- `*Test` (Surefire) / `*IT` (Failsafe) convention; strict enforcer (Java 25, Maven 3.9.9, dependency convergence,
...
1. **No changes under `references/`** — reference projects are read-only inputs. All new work goes under project root
   modules.
2. **Standards and empty modules first** — quality gates, BOMs, and empty module skeletons must build green before any
   source code is migrated.
...
5. **Treat `references/ecs-simulation` and `references/october` as incubators** — defer unless hardened by tests and
   operational evidence.
6. **Single client runtime: LWJGL** — merge both grimoire client codebases into one `grimoire-client-app` using LWJGL
   rendering from `grimoire-client-v2` and OAuth/login/packet coverage from `grimoire-client`.
