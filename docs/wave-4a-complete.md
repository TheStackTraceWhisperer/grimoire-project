...
- **Hand-written fakes over Mockito:** Fakes record invocations with typed inner records — no reflection, no fragile
  `verify()` chains, and better IDE navigation.
- **Compile scope:** Fakes are `compile`-scoped in `test-kit` so consumer modules can use them without re-declaring the
  dependency.
...
1 abstract class in `com.grimoire.testkit.harness` — adapted from the `october` project's harness for Grimoire's
server-side ECS (no OpenGL, no DI container).
...
Both use `ThreadLocalFory` with `Language.JAVA` and `requireClassRegistration(false)` — safe for multi-threaded Netty
pipelines.

### BOM fix

`external-bom` corrected: `org.apache.fury:fury-core` → `org.apache.fory:fory-core` (library rename; version 0.13.1
unchanged).
